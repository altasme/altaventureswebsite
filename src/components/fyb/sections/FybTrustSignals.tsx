import { FYB_TRUST_SIGNALS } from "../../../content/foryourbusiness";
...
// discriminated-union stat shape and useCountUp-driven animation as
// /limitedoffer's OfferHero, but gated on scroll-into-view via useInView
// (useCountUp alone fires on mount) — this band sits below the fold on
// most viewports, so counting up immediately on page load would finish
// before anyone scrolls down to see it.

type Stat = (typeof FYB_TRUST_SIGNALS.stats)[number];
...
function StatValue({ stat, animate }: { stat: Stat; animate: boolean }) {
  ...
  return <p className={`text-3xl font-extrabold tracking-tight text-brand-navy transition-opacity duration-700 sm:text-4xl ${animate ? "opacity-100" : "opacity-0"}`}>{stat.value}</p>;
}

export default function FybTrustSignals() {
