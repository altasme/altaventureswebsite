...
import { B2B_PORTFOLIO } from "../../../content/b2b";
...
export default function B2BPortfolio({ onCheckout }: { onCheckout: () => void }) {
  return (
    <Section id="b2b-portfolio" tone="alt">
      <h2 className="max-w-2xl text-3xl font-bold tracking-tight text-brand-navy sm:text-4xl">
        {B2B_PORTFOLIO.headline}
      </h2>
      <p className="mt-3 max-w-md text-base text-ink/60">{B2B_PORTFOLIO.sub}</p>

      <div className="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {B2B_PORTFOLIO.primaryIds.map((id, i) => (
      ...
      {B2B_PORTFOLIO.advancedIds.length > 0 && (
        <div className="mt-14">
          <p className="text-sm font-semibold uppercase tracking-wide text-ink/40">{B2B_PORTFOLIO.advancedLabel}</p>
          <div className="mt-4 grid gap-6 sm:grid-cols-2">
            {B2B_PORTFOLIO.advancedIds.map((id, i) => (
              <Reveal key={id} delayMs={i * 70}>
                <ProjectCard id={id} />
              </Reveal>
            ))}
      ...
      <div className="mt-10">
        <button type="button" onClick={onCheckout} className="text-sm font-semibold text-brand-blue hover:underline">
          {B2B_PORTFOLIO.cta}
        </button>
      </div>
    </Section>
