...
import { B2B_FAQ } from "../../../content/b2b";
...
export default function B2BFAQ() {
  ...
  return (
    <Section tone="alt">
      <h2 className="max-w-2xl text-3xl font-bold tracking-tight text-brand-navy sm:text-4xl">{B2B_FAQ.headline}</h2>

      <div className="mt-10 max-w-prose divide-y divide-ink/8 border-y border-ink/8">
        {B2B_FAQ.items.map((item, i) => {
          const isOpen = openIndex === i;
          const panelId = `b2b-faq-panel-${i}`;
          const buttonId = `b2b-faq-button-${i}`;
