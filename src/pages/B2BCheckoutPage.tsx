...
import { CHECKOUT } from "../content/b2b";
import { trackInitiateCheckoutB2B } from "../lib/analytics";
import { B2B_PREFILL } from "../lib/contact";
import { ModalProvider, useModals } from "../lib/modalContext";
import ContactModal from "../components/modals/ContactModal";
import LegalModal from "../components/modals/LegalModal";

const PAGE_TITLE = "Start Your ₱4,999 Business Website | Altaventures";
...
function QrPayload({ payload }: { payload: string }) {
  ...
  if (!dataUrl) {
    return (
      <div className="flex h-[280px] w-[280px] items-center justify-center text-sm text-ink/40">
      ...
      {result.kind === "test-placeholder" ? (
        ...
        <>
          ...
          <h2 className="mt-1 text-xl font-bold text-brand-navy">
            {CHECKOUT.price} &middot; Reference {result.referenceNumber}
          </h2>
          ...
          <p className="mx-auto mt-4 max-w-sm text-sm text-ink/60">
            Scan this code with your GCash, Maya, or banking app to complete your {CHECKOUT.price} payment.
          </p>
        </>
      )}
    </div>
      ...
      <div className="space-y-5">
        ...
        <div className="space-y-3 rounded-2xl border border-ink/10 bg-paper-alt p-5">
          <label className="flex items-start gap-3 text-sm text-ink/75">
            ...
            />
            <span>
              I have read and agree to the <LegalLink label="Terms of Sale" onOpen={() => openLegal("b2b-terms")} /> and{" "}
              <LegalLink label="Refund Policy" onOpen={() => openLegal("b2b-refund")} />.
            </span>
          </label>
          <label className="flex items-start gap-3 text-sm text-ink/75">
            <input
            ...
            <span>
              ...
              <LegalLink label="Privacy Notice" onOpen={() => openLegal("b2b-privacy")} />, to deliver this service.
            </span>
          </label>
        </div>
        ...
        {errorMessage && (
          <div className="rounded-xl bg-red-50 px-4 py-3">
            ...
            <button
              type="button"
              onClick={() => openContactModal("checkout-error", B2B_PREFILL)}
