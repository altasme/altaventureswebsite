...
**Known open items (not blocking, do not build speculative code for these):**
- **Payment collection is live (test mode); everything after it is still unbuilt.** The formalized post-payment flow (§7: account-creation email → Supabase Auth account → schedule Setmona discovery call first → build → present/review → ₱1,499 domain upsell → launch → Essential upsell) needs Supabase, Setmona, and full Resend account-flow credentials, none of which exist yet. Today, a confirmed webhook only emails the internal notify address with the payload; a real human still has to follow up manually to onboard the client. Do not build the Supabase/Setmona scaffolding speculatively before those credentials exist — same principle as before, just narrower in scope now that checkout itself is real.
- The FAQ's "What payment methods are accepted?" answer (GCash, Maya, cards) and "What happens after I pay?" answer describe ganap.net's real hosted checkout and the eventual account/call flow; the account/call half of that promise isn't built yet (see above), so a paying customer will get a manual follow-up rather than an automated email + booking link until that's built.
...
- **No order persistence.** There's no database yet, so `functions/testpayment.ts` can't dedupe a webhook retry or query "did this customer pay" later — it's stateless, email-only. Fine at today's volume; revisit once Supabase (or even just a KV namespace) is in the picture.
