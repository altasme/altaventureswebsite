...
**Known open items (not blocking, do not build speculative code for these):**
- **Checkout/payment/backend are unbuilt**, as described above — this is the main gap versus the full spec's funnel (`landing → checkout → payment → account → onboarding → discovery call → build → review → domain upsell → launch → business systems upsell`). Only the landing page half exists. The FAQ's "What payment methods are accepted?" answer (GCash, Maya, cards) describes the intended future checkout, not something a visitor can actually use yet — every CTA still opens the chat modal in the interim.
