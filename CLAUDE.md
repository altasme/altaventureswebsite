...
## 29. `/foryourbusiness` mobile hero fixed at the layout level, not with more crop tuning [2026-09-18]

§24's mobile hero fix (`object-[50%_100%]` bottom-anchoring, same value as the homepage's own mobile hero) did not work — the operator sent a real screenshot of the deployed page showing the CTA button still sitting directly over the subject's face, same failure as the original `object-center` attempt. Two blind attempts at tuning `object-position`/gradient values against a photo this sandbox can never load (same `res.cloudinary.com` block throughout) both failed the same way, which means the problem isn't the specific crop value — it's that the source photo apparently doesn't have enough headroom above the subject for any object-position value to create clearance that doesn't exist in the composition.

**Fixed structurally instead of with more guessing.** The mobile hero (`FybHero.tsx`) no longer overlays text on top of a full-bleed photo at all. It's now two stacked, non-overlapping blocks: a solid `bg-brand-navy-deep` text block sized to its own content (headline, sub, CTA — same `HeroCopy` component, unchanged), followed by a separate `aspect-[4/5]` image block below it. Since the two occupy distinct DOM regions in normal document flow rather than sharing one absolutely-positioned viewport, the button cannot land on the subject's face regardless of how the photo is actually framed — this doesn't depend on knowing the photo's composition at all, unlike every previous attempt. The desktop hero (side-by-side columns, no reported issue) is untouched.

**Trade-off, stated plainly:** this is no longer the homepage's exact full-bleed-photo-with-scrim mobile treatment — it's a more conventional "copy block, then photo block" stack. Given two attempts at matching the homepage's technique both failed against this specific photo, and a third blind guess isn't a responsible use of the operator's time, correctness (guaranteed no overlap) was prioritized over matching the homepage's exact visual style on mobile.

**How this was tested:** `npm run build` and `npm run lint` passed clean. Playwright-verified the geometry directly (not just visually): queried the rendered bounding boxes and confirmed the image block starts at a fixed distance below the page top with the text block (headline through CTA) entirely above it, structurally guaranteeing zero overlap. The actual photo still cannot be visually verified from this sandbox — ask the operator to confirm the new stacked layout looks right once deployed, and note that it now reads as "text, then photo" rather than "photo with text overlaid," a deliberate change from the original full-bleed intent.
