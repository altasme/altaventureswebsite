...
- **`META_PIXEL_ID` is still blank** (shared `lib/analytics.ts` constant) — `trackInitiateCheckoutB2B()` is wired with a `value: 4999` Pixel event but no-ops until an ID is set, same as every other offer on this site.
