...
- All 8 projects now ship real client-supplied images in `public/images/projects/` (no placeholders remain). Setmona and Kolekta use branded logo cards rather than dashboard screenshots, since both are internal engines with no public URL to screenshot; the other six are real live-site screenshots.
...
- The hero's "real businesses built" stat animates 0→8 on load via `src/lib/useCountUp.ts` (ease-out, jumps straight to 8 under `prefers-reduced-motion`) and renders as "8 and counting"; the other two hero stats ("4-7 days", "₱0 dev cost") stay static since they aren't clean countable integers.
