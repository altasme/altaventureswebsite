...
**Visual design is intentionally distinct from the main site [bold "direct-response" restyle].** The homepage's light/dark alternating sections and pill-shaped CTAs are the main-site look; `/limitedoffer` instead runs a fully dark, high-contrast "guru-style" funnel treatment: every section is dark navy, alternating between `brand-navy-deep` and `brand-navy` for rhythm (no light sections anywhere on this route), headlines are bold/extrabold and larger than the main site's, and each section leads with a small blue-square "eyebrow" label. This is deliberately scoped to this route only — `src/components/offer/ui/{OfferSection,Eyebrow,OfferCTAButton,OfferCard}.tsx` are `/limitedoffer`-only primitives, parallel to but never replacing the shared `src/components/ui/{Section,CTAButton,Tag}.tsx` the rest of the site uses, so this restyle cannot leak onto the homepage. CTA buttons on this route are `rounded-lg` (not the main site's `rounded-full`) with a bold uppercase label and an arrow parsed off a trailing "→" in the copy string. The hero has no eyebrow line (per spec) and instead gets an abstract decorative background (radial gradient blobs + a faint grid) built in pure CSS/SVG — never a fabricated product screenshot, consistent with the launch-integrity guardrail (§16).
