...
**Account creation is WorkOS AuthKit, not Supabase Auth, also TEST MODE.** A WorkOS test-mode API key was provided; AuthKit's hosted login page (email/password, magic link, whatever's enabled in the WorkOS dashboard) handles the post-payment "Create Your Account" step from the spec's formalized flow (§7). Setmona (call scheduling) still has no credentials and remains unbuilt — see "Known open items" below.
- **`functions/api/auth-start.ts`** (`GET /api/auth-start`): redirects to WorkOS's hosted authorize URL (`https://api.workos.com/user_management/authorize?client_id=...&redirect_uri=...&response_type=code&provider=authkit`). `client_id` isn't secret (same idea as a Stripe publishable key) but is still read from `env.WORKOS_CLIENT_ID` so there's one source of truth.
- **`functions/api/auth-callback.ts`** (`GET /api/auth-callback`, the registered `redirect_uri`): exchanges the `code` WorkOS sends back for the authenticated user (`POST https://api.workos.com/user_management/authenticate` with `grant_type: "authorization_code"`, `client_id`, `client_secret` = `env.WORKOS_API_KEY`, `code` — server-only, the secret never reaches the client), upserts a `customers` row in D1 (matched to the most recent `orders` row with the same email, best-effort), then redirects to `/foryourbusiness/thank-you?welcome=1` (or `?error=auth_failed` on any failure). **API shape verified against the WorkOS Node SDK's own source on GitHub** (`workos/workos-node`), not workos.com's docs directly — this sandbox's egress proxy blocks `workos.com` outright, so the docs site itself couldn't be fetched.
- This does **not** set up a persistent login session/cookie — there's no account dashboard yet to protect, so there's nothing to keep a session for. Add that once a real post-payment dashboard exists.
- **Required Cloudflare Pages environment variables** (dashboard only, never committed): `WORKOS_API_KEY` (provided, test mode), **`WORKOS_CLIENT_ID` (not yet provided — get this from the WorkOS dashboard; nothing in the auth flow works until it's set)**. Also register `https://altasme.com/api/auth-callback` as an allowed redirect URI in the WorkOS dashboard's Redirects settings, or WorkOS will reject the authorize request.
- **How this was tested:** `workos.com` (including the API host `api.workos.com`) is blocked by this sandbox's egress proxy, so the real authorize/authenticate calls couldn't be exercised. What *was* verified: the authorize URL `/api/auth-start` builds is correctly formed and URL-encoded (checked via the redirect `Location` header, never actually following it); `/api/auth-callback` correctly redirects to the error state when `code` is missing; the D1 upsert logic was exercised directly against a local D1 instance. Test the real WorkOS handshake once `WORKOS_CLIENT_ID` is set and deployed.

- **`src/pages/ForYourBusinessCheckoutPage.tsx`** (`/foryourbusiness/checkout`, `noindex`): collects full name, business name, email, mobile (required) and Facebook/Instagram/existing-website (optional), per spec §21, plus the two required consent checkboxes (Terms of Sale + Refund Policy; Privacy Notice), unticked by default, each linking to the relevant `LegalModal` doc. POSTs to `/api/checkout`; on success, `window.location.href = redirectUrl` sends the browser to ganap.net's hosted payment page. Fires `trackInitiateCheckout()` (dataLayer `checkout_started` + a guarded Meta `InitiateCheckout`, no-ops until `META_PIXEL_ID` is set) on submit.
- **`functions/api/checkout.ts`** (`POST /api/checkout`): validates the form server-side, builds the ganap.net checkout request body (`projectUuid`, `amount`, a fresh `idempotencyKey` per attempt, `customerName`, `customerEmail`, `metadata`, `returnUrl: https://altasme.com/foryourbusiness/thank-you`), signs it with HMAC-SHA256 hex using `env.GANAP_SECRET` (verified byte-for-byte against the vendor's own curl example during this build), POSTs to `https://convex-top-api.ganap.net/v1/checkout`, and returns `{ redirectUrl }`. **Amount-unit assumption:** ganap.net's docs show `"amount":1000` without stating the unit; this sends ₱299 as `29900`, assuming centavos (common PH gateway convention). Confirm against a real test transaction — if wrong, it's the one-line `AMOUNT_PHP_CENTAVOS` constant in that file.
  - **Known issue, confirmed against a real test-mode transaction:** the "Test alta webhoo" ganap.net project returns a `redirectUrl` using a custom, non-`http(s)` scheme (observed: `ganap-test-do-not-pay:REFERENCE`), which browsers can't navigate to — `window.location.href = redirectUrl` silently fails with a console error ("scheme does not have a registered handler"). This function now validates the scheme and returns a clear 502 instead of handing the browser a dead link, and logs the full raw ganap.net response (success or failure) so it's visible in Cloudflare's Functions logs. The placeholder scheme's naming strongly suggests this is deliberate sandbox behavior on ganap.net's side (a "do not actually pay" marker), not a bug in this integration — but there's no public ganap.net documentation to confirm what triggers a real hosted-checkout URL instead (a different project setting? a different `paymentType`? contact their support). Unresolved until ganap.net clarifies.
- **`functions/testpayment.ts`** (`POST /testpayment`, matching the exact webhook path ganap.net is configured to call): the authoritative, server-to-server payment confirmation — the browser landing on the thank-you page is only a UX nicety and is never trusted alone. Verifies `X-Ganap-Signature` (HMAC-SHA256 hex of the raw body, same secret) before doing anything, returning 401 on a missing/forged signature (verified with both a validly-signed and a forged payload during this build). **Assumption flagged in the file:** ganap.net's docs only document signing outgoing checkout requests this way; incoming webhook verification assumes the same scheme. If a D1 database is bound, it also tries to match the webhook back to the `pending` order `functions/api/checkout.ts` created (by `externalReference`/`referenceNumber`/`id` against the order's own id, which is the `idempotencyKey` sent to ganap.net) and marks it paid with the raw payload attached; no match (or no DB bound) isn't an error, since it always emails the full raw payload to the internal notify address via Resend (reusing `RESEND_API_KEY`/`RESEND_FROM_EMAIL` from the WSA-free function) regardless, so a human sees every event while the real payload shape is confirmed against live test transactions.
- **`src/pages/ForYourBusinessThankYouPage.tsx`** (`/foryourbusiness/thank-you`, `noindex`, the `returnUrl` target): deliberately hedged copy ("if your payment went through...") rather than claiming a confirmed purchase, since only the webhook can confirm that. Fires `track("payment_return", {})` on mount as a dataLayer marker, not a Meta `Purchase` event — a client-side Purchase fire off an unverified browser redirect would inflate ad reporting with unconfirmed conversions, so that's deliberately not done here. Below the payment card it now also shows the "Create Your Account" panel (links to `/api/auth-start`), which swaps to a "You're All Set!" confirmation when the URL carries `?welcome=1` (set by `auth-callback.ts` on success) or an inline error when it carries `?error=auth_failed`. Includes a "Message Us" fallback into `ContactModal` for anyone who hits trouble either way.
- **Required Cloudflare Pages environment variables** (dashboard only, never committed): `GANAP_SECRET`, `GANAP_PROJECT_UUID`, `WORKOS_API_KEY`, `WORKOS_CLIENT_ID` (see below). `functions/testpayment.ts` additionally needs the already-required `RESEND_API_KEY`/`RESEND_FROM_EMAIL` to send its notification email (optional `PAYMENT_NOTIFY_EMAIL`, defaults to `altasmeworks@gmail.com`). Optional: `DB` (the D1 binding, see below).
- **How this was tested:** this sandbox's egress proxy blocks `convex-top-api.ganap.net` outright (`403 Host not in allowlist`), so the real end-to-end payment call could not be exercised from here. What *was* verified locally via `wrangler pages dev` with a gitignored `.dev.vars` and a local D1 instance bound via a gitignored `wrangler.toml` (both deleted after testing, never committed): the HMAC signature this code produces matches `openssl`'s output for the vendor's own example body+secret; `/api/checkout` correctly returns 400s for each invalid-input case, a graceful 502 when the real network call fails, and — confirmed against a real local D1 database — inserts a `pending` order row before that network call, so the order exists even though the ganap.net call itself couldn't be exercised; `/testpayment` accepts a correctly-signed test payload (200) and rejects both a forged and a missing signature (401 both times), and correctly updates the matching D1 order to `paid` with the webhook payload attached. Test the real checkout redirect end-to-end from an environment with real network access (e.g. after deploying, or from your own machine) before relying on it live.
...
- **Primary grid:** `dmhr`, `vocalyze`, `aulea`, `pocketg7iii`, `macquias` — five real, live, simple `site`-tier projects at ₱299 scale, all with working "View Website" links to real screenshots already in `public/images/projects/`.
...
**Hero visual is a labeled placeholder, not a fabricated mockup.** The spec (§8) calls for a real device composition (one dominant desktop shot + 1-2 mobile shots of real Altaventures work) that doesn't exist as an asset yet. Per the site-wide no-fabricated-screenshots guardrail (§16), `src/components/fyb/FybHeroVisual.tsx` renders a dashed-border placeholder box instead, labeled with the exact spec to build to: **canvas 1600 × 1200px, aspect ratio 4:3, PNG with transparent background**. Swap this component's contents for the real asset once produced; nothing else on the page needs a placeholder since the portfolio section reuses real, already-captured screenshots.
...
**Known open items (not blocking, do not build speculative code for these):**
- **`WORKOS_CLIENT_ID` is the one missing piece blocking the account-creation flow end to end.** `WORKOS_API_KEY` was provided; the client ID (public, from the WorkOS dashboard) wasn't. Until it's set, `/api/auth-start` returns a friendly "not configured" error instead of redirecting to WorkOS — same graceful-degradation pattern as the other integrations on this site.
- **The D1 database itself still needs to be created and bound.** Nothing above persists anywhere in production until someone creates a D1 database in the Cloudflare dashboard (or `wrangler d1 create`), binds it to this Pages project with variable name `DB`, and runs `d1/schema.sql` against it. Until then, `env.DB` is `undefined` everywhere and every DB-touching code path silently no-ops (by design — a missing DB should never break a payment).
- **Setmona (call scheduling) still has no credentials and remains unbuilt.** The formalized post-payment flow (§7: payment → account → discovery call → build → present/review → ₱1,499 domain upsell → launch → Essential upsell) now has its first two steps live (payment, account creation); scheduling the discovery call is the next gap. Do not build Setmona scaffolding speculatively before credentials exist.
- The FAQ's "What payment methods are accepted?" answer (GCash, Maya, cards) and "What happens after I pay?" answer describe ganap.net's real hosted checkout and the eventual account/call flow; the call-scheduling half of that promise isn't built yet (see above), so a customer who creates an account still needs a manual follow-up to actually book their discovery call until Setmona is wired up.
- **Hero visual placeholder** needs a real device composition; see spec above.
...
- **No persistent login session.** `auth-callback.ts` creates/updates the WorkOS user and the D1 `customers` row, then redirects to a one-time confirmation — it does not set a session cookie, since there's no account dashboard yet to protect. Add real session handling once a dashboard exists to log back into.
- SEO/OG tags are set client-side in `ForYourBusinessPage.tsx`'s effect, same JS-SPA caveat as `/limitedoffer` (§18) — a non-JS crawler sees the homepage's static tags instead. No dedicated OG image supplied yet; inherits `/og/altaventures-og.jpg`.
