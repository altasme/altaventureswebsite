...
## 19. ₱299 WEBSITE OFFER LANDING PAGE (`/foryourbusiness`)

A paid-social landing page for the ₱299 one-time professional website offer, built from the authoritative spec `ALTAVENTURES___ForYourBusiness.txt`. Route: `src/pages/ForYourBusinessPage.tsx`, lazy-loaded in `App.tsx` like `/limitedoffer` and `/WSA-free`, so its content never ships in the homepage bundle. Content lives in `src/content/foryourbusiness.ts` (one exported const per section, following the same single-source-of-truth pattern as `content/offer.ts`), reusing `BRAND`/`CONTACT` from `content/site.ts` and portfolio data from `content/portfolio.ts` rather than duplicating either.

**Checkout/payment is deliberately deferred, not built.** The full spec (and `CLAUDEforyourbusiness.md`) describes a ganap.net payment integration, a Supabase-backed account/onboarding/scheduling flow, Setmona call booking, and Resend email, none of which have credentials available in this environment. Building that scaffolding blind (with placeholder env vars that can never actually authenticate) would ship something that looks functional but silently can't process a real payment — worse than not building it. Instead, **every CTA on this page opens the same `ContactModal` used site-wide** (channel picker: WhatsApp/Messenger/Viber), with a ₱299-specific WhatsApp prefill (`FYB_PREFILL` in `lib/contact.ts`: *"Hi Altaventures! I'd like to get the ₱299 website offer for my business."*). This keeps the page a real, working, chat-qualified lead path today. When ganap.net/Supabase/Setmona/Resend credentials are available, replace the CTA handlers with real navigation to `/foryourbusiness/checkout` and build out the post-payment flow per the spec's §21-§29 — do not build that scaffolding speculatively before credentials exist.

**Modal plumbing added to support this:** `ModalContextValue.openContactModal` now takes an optional second `prefillMessage` argument (stored as `contactPrefill` alongside `contactSection`), and `ContactModal.tsx`'s WhatsApp channel calls `whatsappUrl(contactPrefill)` instead of the no-arg default. Passing nothing preserves the original site-wide default prefill (`CONTACT.whatsapp.prefill`) for every existing caller, so this is additive, not a behavior change elsewhere.
...
**Page-wide motion reuses `/limitedoffer`'s `Reveal` component directly** (`src/components/offer/Reveal.tsx`, imported cross-folder rather than duplicated or relocated) — every section below the hero is wrapped in it, with staggered `delayMs` on grid items (Value cards, scenario cards, portfolio cards, how-it-works steps), matching the established pattern of that page.

**Section order (13 sections + footer, `ForYourBusinessPage.tsx`):** Header → Hero → Reality → Social Media Reality → Value ("What ₱299 Actually Gets You") → What This Looks Like (4 scenarios) → Portfolio → How It Works → Doesn't Include → Why ₱299 → Urgency (brand-tone, lists real `ONGOING` projects as capacity evidence, no timers or fake slot counts per the spec's explicit prohibition) → Guarantee → FAQ (9 items) → Final CTA → Footer (logo, tagline, the three fyb legal links, copyright) → sticky mobile CTA (`₱299 · GET MY WEBSITE →`).
...
**Known open items (not blocking, do not build speculative code for these):**
- **Checkout/payment/backend are unbuilt**, as described above — this is the main gap versus the full spec's funnel (`landing → checkout → payment → account → onboarding → discovery call → build → review → domain upsell → launch → business systems upsell`). Only the landing page half exists.
- **Hero visual placeholder** needs a real device composition; see spec above.
- `META_PIXEL_ID` is blank (shared `lib/analytics.ts` constant) — the spec's Pixel events (`PageView`, `WebsiteOfferCTA`, `InitiateCheckout`, `Purchase`) aren't wired for this route since there's no real checkout to fire `Purchase` from yet.
- Legal content needs a Philippine lawyer's review before the ₱299 offer processes real payments, per `foryourbusinesspolicies.md`'s own disclaimer.
