...
### 30.8 Payment split corrected again: ₱2,499 deposit / ₱2,500 balance, not an even 50/50 [2026-09-19, same day]

The operator corrected §30.6's even ₱2,499.50/₱2,499.50 split: it's actually **₱2,499 to start and ₱2,500 on completion** — an uneven split, not exactly 50/50 (₱2,499 is ~49.99% of ₱4,999). Every figure introduced in §30.6 was updated to match:

- **`content/b2b.ts`**: `DEPOSIT_PHP` changed `"₱2,499.50"` → `"₱2,499"`, `BALANCE_PHP` changed `"₱2,499.50"` → `"₱2,500"`. Every string built from these constants (CTA labels, hero sub, `WHATS_INCLUDED.scopeLine`, `B2B_HOW_IT_WORKS`, `B2B_FINAL_CTA.body`, `CHECKOUT`, `THANK_YOU`, `B2B_FAQ`) updated automatically since they were already routed through the constants rather than hardcoded a second time — this is exactly the payoff §30.6 called out for centralizing the figures that way. The one string that needed a manual edit was `CHECKOUT.priceNote`, which had literally said `"50% DEPOSIT · ${TOTAL_PHP} TOTAL"`; since the split is no longer exactly 50/50, that claim is now false, so it was reworded to `"DEPOSIT · ${TOTAL_PHP} TOTAL"` (no percentage claim at all) rather than left stating an inaccurate number.
- **`functions/api/checkout.ts`**: `OFFER_CONFIG.b2b.amountPhp` changed `2499.5` → `2499` — this endpoint now charges the ₱2,499 deposit, not the ₱2,499.50 figure from the first correction. `metadataOffer` (`"b2b-4999-deposit"`) was already split-amount-agnostic, so no change needed there. Re-verified via the same local `wrangler pages dev` + temporary diagnostic-log method as both prior payment-flow checks (§30.4, §30.6): confirmed `offer: "b2b"` now resolves `amountPhp: 2499` before the network call, removed the temp log and `.dev.vars`/`.wrangler` afterward.
- **`content/site.ts`**: every `₱2,499.50` mention across `b2b-refund` (the intro, "Before Work Begins," "After the Discovery Call," "The Balance Payment," "Client Responsiveness," "Annual Renewal and Cancellation") and `b2b-terms` ("Payment: Deposit and Balance") reworded to the correct deposit or balance figure as context required — a deposit-refund sentence now says ₱2,499, a balance-due sentence now says ₱2,500. The Limitation of Liability section in `b2b-terms` needed no change: it already said "the sum of your deposit and balance" generically rather than restating the specific split figures.
- **`src/lib/analytics.ts`**: `trackInitiateCheckoutB2B()`'s Meta Pixel `value` changed `2499.5` → `2499`, matching the actual deposit this checkout step now charges.

**How this was tested:** `npm run build` and `npm run lint` passed clean. Grepped the entire repo for `2,499.50`/`2499.5` afterward and confirmed zero remaining hits anywhere. Re-ran the local `wrangler pages dev` deposit-amount check, confirming `offer: "b2b"` resolves `amountPhp: 2499`. Screenshot-verified via a local `vite preview` + Playwright pass that the hero and checkout page both show "₱2,499" as the amount charged today, "₱2,500 balance due on completion" in the checkout summary, and the price note reads "DEPOSIT · ₱4,999 TOTAL" with no percentage claim; confirmed via body-text substring checks on both pages that `2,499.50` appears nowhere while `2,499`, `2,500`, and `4,999` all appear correctly. All scratch tooling (`playwright-core`, the screenshot script, `.dev.vars`, `.wrangler`, the temporary diagnostic log) removed after verification.
