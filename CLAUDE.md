...
**Visual design reuses the main site's components directly [corrected after an eye-strain / AI-slop pass].** An earlier iteration gave this route its own fully-dark, all-navy "guru funnel" treatment with a small blue-square eyebrow label above every heading and a blurred radial-gradient-and-grid hero background. A design review against a concrete AI-slop checklist (kicker/eyebrow labels above headings, decorative radial-gradient halos, decorative grid-line backgrounds) flagged all three as recognizable AI-generated-UI tells, and reading a long page entirely in white-on-navy was also flagged as harder on the eyes than the main site's alternating light/dark sections. Both problems had the same fix: **`/limitedoffer` now imports and uses the exact same `src/components/ui/{Section,CTAButton,Tag}.tsx` the homepage uses** — no parallel offer-only UI kit exists anymore. Sections alternate `light` / `alt` / `dark` tone in a rhythm similar to the homepage's own (roughly 3 dark anchor sections out of 13: `Agitation`, `Scarcity`, `OfferFinalCTA`; the rest light or alt), pill-shaped `CTAButton`s, and no eyebrow labels anywhere, including the hero (which also has none per the copy spec). The hero background is a solid `brand-navy-deep` panel with two small, crisp-edged (no blur, no radial gradient) corner accents clipped from `bg-brand-gradient` at low opacity — the brand gradient used exactly as CLAUDE.md §11.1 sanctions ("hero accent... use sparingly"), not as a glow effect. What still makes this route feel distinct from the homepage is the copy and section structure (the direct-response funnel flow below), not a separate visual system.
